...
test.describe.serial("MCP server details and access control", () => {
  ...
  test.afterAll(async () => {
    if (editorServerId) {
      await deleteMcpServer({ page: adminPage }, editorServerId).catch((err) =>
        console.warn("Cleanup: could not delete MCP server", err),
      );
    }
