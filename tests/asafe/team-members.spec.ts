...
test.describe.serial("Team Members API", () => {
  ...
  test("admin: POST /api/admin/teams/[id]/members — add editor by email", async () => {
    ...
    const res = await adminPage.request.post(`/api/admin/teams/${teamId}/members`, {
      headers: { "Content-Type": "application/json" },
  ...
  test("admin: GET /api/admin/teams/[id]/members — lists added member", async () => {
    ...
    const res = await adminPage.request.get(`/api/admin/teams/${teamId}/members`);
    expect(res.status(), `expected 200, got ${res.status()}`).toBe(200);
    ...
    const member = body.members.find(
      (m: { email: string; id: string }) => m.email === TEST_USERS.editor.email,
    );
  ...
  test("admin: POST — re-add same user with different role (idempotent update)", async () => {
    ...
    expect(res.status(), `expected 200 for role update, got ${res.status()}`).toBe(200);
  });
