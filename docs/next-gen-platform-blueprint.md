...
- **Code/PR agents: don't build them — embed one.** Engineering uses Agent HQ/Claude Code natively; asafe-ai's job is the GitHub connector in the catalog and ingesting agent-authored results via webhook — not competing with four-vendor coding platforms. The exception is the **desktop**: embed the open-source **opencode SDK** (`@opencode-ai/sdk`, local server + client API) in the Electron app as a governed coding surface — its model calls routed through asafe-ai's OpenRouter gateway so engineering coding sessions inherit the same entitlements, budgets, and audit as chat, and local execution stays on the engineer's machine consistent with the desktop local-execution policy (ADR-0010 gates apply).
...
## Realtime & collaboration (ElectricSQL plan — Notion-style teamspaces)

The collaboration model is **Notion, not Claude Projects**: shared folder trees ("teamspaces") owned by a team, with conversations living inside folders and permissions inheriting downward (teamspace → folder → thread), entitlement checks on content still applying. Sharing a folder means teammates see its conversations appear and update **live**, and multiple people + the AI can participate in the *same* conversation in realtime (presence, typing indicators, everyone sees the model stream).

Electric is **strictly read-path-only — every write goes through the server layer per the CLAUDE.md rule (Server Actions for UI mutations, routes for streaming), never through the sync layer** — so guardrails/budget/audit are never bypassed and each human message in a multiplayer chat is attributed, audited, and budget-charged under its author's identity. Phasing: (1) link-shared **read-only thread snapshots** with Private/Team visibility — Team checks recipient team membership *and* the entitlement/KB scope of thread content — no Electric needed; (2) Electric sidecar Deployment in the Helm chart reading Postgres logical replication (no Redis), shapes scoped by folder/team membership for the Runs rail, folder trees, Triage unread counts, and Electron badges; (3) full multiplayer: live shared conversations with presence inside collaborative folders — only after (2) proves the sync layer in production.
