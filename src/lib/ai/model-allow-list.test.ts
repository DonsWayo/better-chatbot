...
describe("W4 model allow-list enforcement predicate", () => {
  ...
  it("allows a model that is in the allow-list", () => {
    const list = ["gpt-5.5", "gemini-3.5-flash"];
    expect(isModelAllowed("gpt-5.5", list)).toBe(true);
    expect(isModelAllowed("gemini-3.5-flash", list)).toBe(true);
  });

  it("blocks a model that is not in a non-empty allow-list", () => {
    const list = ["gpt-5.5"];
    expect(isModelAllowed("claude-opus-4.8", list)).toBe(false);
    expect(isModelAllowed("gemini-3.5-flash", list)).toBe(false);
  });
  ...
  it("single-model allow-list blocks all other approved models", () => {
    const list = ["gemini-3.1-flash-lite"];
    const others = ["gpt-5.5", "claude-opus-4.8", "gemini-3.5-flash"];
    for (const m of others) {
      expect(isModelAllowed(m, list)).toBe(false);
    }
    expect(isModelAllowed("gemini-3.1-flash-lite", list)).toBe(true);
  });
...
const APPROVED_MODEL_IDS = ["gpt-5.5", "claude-opus-4.8", "gemini-3.5-flash", "gemini-3.1-flash-lite"];

describe("APPROVED_MODEL_IDS contract", () => {
  it("contains exactly the four approved models", () => {
    expect(APPROVED_MODEL_IDS).toHaveLength(4);
    expect(APPROVED_MODEL_IDS).toContain("gpt-5.5");
    expect(APPROVED_MODEL_IDS).toContain("claude-opus-4.8");
    expect(APPROVED_MODEL_IDS).toContain("gemini-3.5-flash");
    expect(APPROVED_MODEL_IDS).toContain("gemini-3.1-flash-lite");
  });
...
describe("W4 model allow-list enforcement predicate — edge cases", () => {
  ...
  it("returns true for undefined modelId with single-item list", () => {
    expect(isModelAllowed(undefined, ["gemini-3.5-flash"])).toBe(true);
  });

  it("is not case-insensitive — exact match required", () => {
    expect(isModelAllowed("Gpt-5.1", ["gpt-5.5"])).toBe(false);
  });
  ...
  it("blocks all APPROVED_MODEL_IDS when list contains only one model", () => {
    const singleList = ["gpt-5.5"];
    const others = APPROVED_MODEL_IDS.filter((m) => m !== "gpt-5.5");
    for (const m of others) {
  ...
  it("returns false for a model not in a large allow-list", () => {
    const list = ["gpt-5.5", "claude-opus-4.8", "gemini-3.5-flash"];
